...
class BookController extends Controller
{
    ...
    public function ordercreate(Request $request) {
        ...
        $orderId = DB::transaction(function () use ($request, $cart) {
            $books = DB::table('sach')->whereIn('id', array_keys($cart))->get();
            $tongTien = 0;
            foreach($books as $b) 
                { $tongTien += $b->gia * $cart[$b->id]; }
            ...
            try {
                Mail::to(Auth::user()->email)->send(new OrderSuccessNotification($mailData));
            } catch (\Exception $e) {}
            return $idDH;
        }});
        session()->forget('cart');
        return redirect()->route('order')->with('status', 'Đặt hàng thành công! Email đã được gửi. Mã đơn: #' . $orderId);
    }
